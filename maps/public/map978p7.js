...
var stn_sql = url.concat("SELECT t.stn_call, CONCAT(s.stn_loc,', ', s.state) AS stn_loc,\
			ob_date, CONCAT('Tp: ',temperature,'°F <br>Dp: ',dewp,'°F') AS temperature, \
			concat((CASE WHEN windsp <> '-' THEN CONCAT(windsp,'kts<br>') \
			ELSE windsp END), (CASE WHEN winddir = 'Variable' THEN winddir \
			WHEN winddir <> '-' THEN concat(winddir,'deg') \
			ELSE winddir END)) AS windsp, \
			CONCAT(altimeter,' Ins<br>',slp,' hPa') AS slp, \
			CASE WHEN visby <> '-' THEN concat(visby,' sm') \
			ELSE visby END AS visby, \
			dewp, hrly_precip, windgust, mtype, concat(cld_type1,'<br>', cld_type2,'<br>', \
			cld_type3,'<br>', cld_type4, '<br>',cld_type5) as cld_type1,\
