...
//** SIGMET
L.realtime({url: 'http://localhost:8000/sigmet.geojson',
	crossOrigin: true, type: 'json'},  {interval: 3 * 1000,
		style: function(feature){
// var fillColor = '#' +Math.floor(Math.random()*16777215).toString(16);
			var fillColor;
		...
		onEachFeature: function(feature,layer){
			layer.bindPopup('SIGMET: Rep ID:' + feature.properties.RepNum
					+ '<br>Altitude: ' + feature.properties.Alt	
			)}}
).addTo(map);

//** G-AIRMET
L.realtime({url: 'http://localhost:8000/gairmet.geojson',
	crossOrigin: true, type: 'json'},  {interval: 3 * 1000,
		style: function(feature){
// var fillColor = '#' +Math.floor(Math.random()*16777215).toString(16);
			var fillColor;
